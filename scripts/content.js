...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        ...
        } else if (request.type === 'allTabs') {
            copyAllTabs(request, sender, sendResponse);
        }
    }
);
...
function copyAllTabs(request, sender, sendResponse) {
    let msg = document.createElement('ul');
    for (let link of request.links) {
        msg.innerHTML += `<li><a href="${link.url}">${link.title}</a></li>`;
    }
